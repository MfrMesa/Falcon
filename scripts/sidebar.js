...
// Listener para cuando la pantalla sea mayor a 1200px y .sideCollapser sea visible
document.getElementById("toggleNavSidebar").addEventListener("click", () => {
    let sidebarWrap = document.querySelector(".sideCollapser");
    
    // Verificar si la pantalla es mayor a 1200px y .sideCollapser es visible
    if (window.innerWidth > 1200 && getComputedStyle(sidebarWrap).visibility !== "hidden") {
        // Ejecutar el resto del código solamente si se cumple la condición
        let listsToToggle = document.querySelectorAll(".navTitle, .innerDrop");
        listsToToggle.forEach((list) => {
            list.classList.toggle("collapsed");
        });
...
// Listener para cuando la pantalla sea menor a 1200px y .sideCollapser tenga visibilidad oculta
document.getElementById("toggleNavSidebar").addEventListener("click", () => {
    let sidebarWrap = document.querySelector(".sideCollapser");
    
    // Verificar si la pantalla es menor a 1200px y .sideCollapser tiene visibilidad oculta
    if (window.innerWidth < 1200 && getComputedStyle(sidebarWrap).visibility === "hidden") {
        // Ejecutar el resto del código solamente si se cumple la condición
        let listsToToggle = document.querySelectorAll(".navTitle, .innerDrop");
        listsToToggle.forEach((list) => {
